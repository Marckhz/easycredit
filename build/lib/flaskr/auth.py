...
def update_table():
	...
	if request.method =='POST':
		...
		var = db.execute('UPDATE userinfo SET status = ? WHERE id= ?',(go,id)
		).fetchone()
		db.commit()

		return redirect(url_for('auth/dashboard.html'), var=var)
		 

		dont = request.form['deny']
		var2= db.execute('UPDATE  userinfo SET status= ? WHERE id = ?',(dont,id)
		).fetchone()
		db.commit()

		return redirect(url_for('auth/dashboard'), var2=var2)
